...
try:
    from .SwinTransformerV2 import SwinV2B#, SwinS, SwinT
except:
    from model.SwinTransformerV2 import SwinV2B #, SwinS
...
class PositionEmbeddingSine:
    def __init__(self, num_pos_feats=64, temperature=10000, normalize=False, scale=None):
        ...
        if scale is None:
            scale = 2 * math.pi
        self.scale = scale
        # self.dim_t = torch.arange(0, self.num_pos_feats, dtype=torch.float32, device='cpu')
        self.dim_t = torch.arange(0, self.num_pos_feats, dtype=torch.float32, device='cpu')

    def __call__(self, b, h, w):
        mask = torch.zeros([b, h, w], dtype=torch.bool, device='cpu')
        ...
        if self.normalize:
            eps = 1e-6
            y_embed = ((y_embed - 0.5) / (y_embed[:, -1:, :] + eps) * self.scale).cpu()
            x_embed = ((x_embed - 0.5) / (x_embed[:, :, -1:] + eps) * self.scale).cpu()
    ...
    def forward(self, l, g):
        ...
        for pool_ratio in self.pool_ratios:
            ...
            if self.g_pos is None:
                ...
                self.p_poses.append(pos_emb.cuda())
        pools = torch.cat(pools, 0).cuda()
        if self.g_pos is None:
...
class inf_MCLM(nn.Module):  
    ...
    def forward(self, l, g):
        ...
        g_hw_b_c = rearrange(g, 'b c h w -> (h w) b c')
        try:
            g_hw_b_c = g_hw_b_c + self.dropout1(self.attention[0](g_hw_b_c.cuda() + self.g_pos.cuda(), pools.cuda() + self.p_poses.cuda(), pools.cuda())[0])
        except:
            g_hw_b_c = g_hw_b_c + self.dropout1(self.attention[0](g_hw_b_c + self.g_pos, pools + self.p_poses, pools)[0])
...
class MCRM(nn.Module):
    def __init__(self, d_model, num_heads, pool_ratios=[4, 8, 16], h=None):
        ...
        self.activation = get_activation_fn('relu')
        self.sal_conv = nn.Conv2d(d_model, num_heads, 1)
        self.pool_ratios = pool_ratios
        self.positional_encoding = PositionEmbeddingSine(num_pos_feats=d_model // 2, normalize=True)
        self.channel_proj = nn.Conv2d(self.num_heads, self.d_model, kernel_size=1)
    ...
    def forward(self, x):
        ...
        try:
            ...
            loc = loc * rearrange(self.channel_proj(token_attention_map), 'b c (hg h) (wg w) -> (hg wg b) c h w', hg=2, wg=2)
...
class inf_MCRM(nn.Module):
    def __init__(self, d_model, num_heads, pool_ratios=[4, 8, 16], h=None):
        ...
        self.pool_ratios = pool_ratios
        self.positional_encoding = PositionEmbeddingSine(num_pos_feats=d_model // 2, normalize=True)
        self.channel_proj = nn.Conv2d(num_heads, d_model, kernel_size=1)
...
class MVANet(nn.Module):  
    def __init__(self, num_classes=1, emb_dim=128):
        super().__init__()

        self.num_classes = num_classes

        self.backbone = SwinV2B(pretrained=True)
        self.sideout5 =  nn.Sequential(nn.Conv2d(emb_dim, self.num_classes, kernel_size=3, padding=1))
        self.sideout4 = nn.Sequential(nn.Conv2d(emb_dim, self.num_classes, kernel_size=3, padding=1))
        self.sideout3 =  nn.Sequential(nn.Conv2d(emb_dim, self.num_classes, kernel_size=3, padding=1))
        self.sideout2 =  nn.Sequential(nn.Conv2d(emb_dim, self.num_classes, kernel_size=3, padding=1))
        self.sideout1 = nn.Sequential(nn.Conv2d(emb_dim, self.num_classes, kernel_size=3, padding=1))
        ...
        # SWINS
        # self.output5 = make_cbr(768, emb_dim)
        # self.output4 = make_cbr(384, emb_dim)
        # self.output3 = make_cbr(192, emb_dim)
        # self.output2 = make_cbr(96, emb_dim)
        # self.output1 = make_cbr(96, emb_dim)
        ...
        self.multifieldcrossatt = MCLM(emb_dim, self.num_classes, [1, 4, 8])
        ...
        self.dec_blk1 = MCRM(emb_dim, self.num_classes, [2, 4, 8])
        self.dec_blk2 = MCRM(emb_dim, self.num_classes, [2, 4, 8])
        self.dec_blk3 = MCRM(emb_dim, self.num_classes, [2, 4, 8])
        self.dec_blk4 = MCRM(emb_dim, self.num_classes, [2, 4, 8])
        ...
        self.output = nn.Sequential(nn.Conv2d(emb_dim, self.num_classes, kernel_size=3, padding=1))
    ...
    def forward(self, x):
        ...
        ####### concat 4 to 1 #######
        sideout1 = patches2image(sideout1[:-1]).cpu()
        sideout2 = patches2image(sideout2[:-1]).cpu()####(5,c,h,w) -> (1 c 2h,2w)
        sideout3 = patches2image(sideout3[:-1]).cpu()
        sideout4 = patches2image(sideout4[:-1]).cpu()
        sideout5 = patches2image(sideout5[:-1]).cpu()
...
class inf_MVANet(nn.Module):  
    def __init__(self, num_classes=1, emb_dim=128):
        ...
        self.multifieldcrossatt = inf_MCLM(emb_dim, self.num_classes, [1, 4, 8])
        self.conv1 = make_cbr(emb_dim, emb_dim)
        self.conv2 = make_cbr(emb_dim, emb_dim)
        self.conv3 = make_cbr(emb_dim, emb_dim)
        self.conv4 = make_cbr(emb_dim, emb_dim)
        self.dec_blk1 = inf_MCRM(emb_dim, self.num_classes, [2, 4, 8])
        self.dec_blk2 = inf_MCRM(emb_dim, self.num_classes, [2, 4, 8])
        self.dec_blk3 = inf_MCRM(emb_dim, self.num_classes, [2, 4, 8])
        self.dec_blk4 = inf_MCRM(emb_dim, self.num_classes, [2, 4, 8])
